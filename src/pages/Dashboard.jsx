...
const Dashboard = () => {
  ...
  const totalTenders = stats.statusDistribution.active + 
  ...
  return (
    <div className="animate-fadeIn p-2 md:p-6 overflow-x-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-3 sm:space-y-0">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Department Dashboard</h1>
        <div className="bg-white rounded-lg shadow-sm p-1 flex w-full sm:w-auto">
          <button 
            className={`px-2 sm:px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'overview' ? 'bg-sky-100 text-sky-600' : 'text-gray-600 hover:bg-sky-50'}`}
            onClick={() => setActiveTab('overview')}
          >
            Overview
          </button>
          <button 
            className={`px-2 sm:px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'analytics' ? 'bg-sky-100 text-sky-600' : 'text-gray-600 hover:bg-sky-50'}`}
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          <Card title="Total Tenders" value={stats.totalTenders} icon={<FileText size={24} className="text-green-500" />} />
          <Card title="Active Tenders" value={stats.activeTenders} icon={<FileText size={24} className="text-yellow-500" />} />
          <Card title="Total Clashes" value={stats.totalClashes} icon={<AlertTriangle size={24} className="text-red-500" />} />
          <Card title="Total Items in Inventory" value={stats.totalInventory} icon={<Package size={24} className="text-purple-500" />} />
          <Card title="Resolved Issues" value={stats.resolvedIssues} icon={<AlertCircle size={24} className="text-teal-500" />} />
        </div>
      )}
      ...
      {activeTab === 'analytics' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Tenders by Month</h2>
            <div className="h-48 sm:h-64 flex items-end space-x-1 md:space-x-2">
              {stats.tendersByMonth.map((item, index) => (
                <div key={index} className="flex flex-col items-center flex-1">
                  <div 
                    className="w-full bg-sky-500 rounded-t-md transition-all duration-500 ease-out hover:bg-sky-600"
                  ...
                  <div className="text-xs sm:text-sm font-semibold">{item.count}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Clash Resolution</h2>
            <div className="flex flex-col sm:flex-row items-center justify-center">
              ...
              <div className="mt-4 sm:mt-0 sm:ml-6">
                <div className="mb-2">
                  <div className="flex items-center">
                    <div className="w-3 h-3 rounded-full bg-emerald-500 mr-2"></div>
                    <span className="text-xs sm:text-sm">Resolved: {stats.clashResolution.resolved}</span>
                  </div>
                </div>
                <div>
                  <div className="flex items-center">
                    <div className="w-3 h-3 rounded-full bg-gray-200 mr-2"></div>
                    <span className="text-xs sm:text-sm">Unresolved: {stats.clashResolution.unresolved}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 overflow-hidden">
              <div className="flex">
                ...
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-medium truncate">New tender approved</p>
              ...
              <div className="flex">
                ...
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-medium truncate">Clash C002 reported</p>
              ...
              <div className="flex">
                ...
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-medium truncate">Issue #45 resolved</p>
              ...
              <div className="flex">
                ...
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-medium truncate">New inventory items added</p>
